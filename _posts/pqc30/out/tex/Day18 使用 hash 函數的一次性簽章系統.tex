...
\hypertarget{hash-ux51fdux6578}{%
\section{Hash 函數}\label{hash-ux51fdux6578}}

Hash
函數是一種將任意長度的輸入轉換為固定長度輸出的算法，輸出被稱為哈希值（hash
value）。好的哈希函數具有以下特性：

\begin{itemize}
\tightlist
\item
  \textbf{不可逆性}：給定輸出，幾乎不可能反推出輸入。
\item
  \textbf{抗碰撞性}：幾乎不可能找到兩個不同的輸入產生相同的輸出（即哈希碰撞）。
\item
  \textbf{高效性}：計算哈希值的時間應該非常快速。
\end{itemize}

常見的 Hash 函數包括 SHA-256 和 SHA-3，它們在現代密碼學中有著廣泛應用。

讓我們試著使用 Python 來生成 SHA-256 和 SHA-3
的哈希值，你可以將自己的訊息插入進去，看看會產生什麼樣的哈希值。
...
\hypertarget{ux91cfux5b50ux6f14ux7b97ux6cd5ux5c0d-hash-ux51fdux6578ux7684ux5a01ux8105}{%
\subsection{量子演算法對 hash
函數的威脅}\label{ux91cfux5b50ux6f14ux7b97ux6cd5ux5c0d-hash-ux51fdux6578ux7684ux5a01ux8105}}

在量子計算的背景下，Grover's Algorithm
能夠加速暴力破解哈希函數。傳統暴力攻擊的複雜度是 \(O(2^n)\)，而使用
Grover's Algorithm，複雜度下降到 \(O(2^{n/2})\)。這意味著如果使用 256
位的哈希函數，在量子電腦下的安全性相當於只有 128
位。但也不必太過擔心，只要把安全參數增加一倍即可擁有與之前一樣的安全性

\hypertarget{ld-ots-ux4e00ux6b21ux6027ux7c3dux540dux65b9ux6848}{%
\section{LD-OTS
一次性簽名方案}\label{ld-ots-ux4e00ux6b21ux6027ux7c3dux540dux65b9ux6848}}

\hypertarget{ux5bc6ux9470ux751fux6210}{%
\subsection{密鑰生成}\label{ux5bc6ux9470ux751fux6210}}
...
\end{aligned}
\] 因為我們使用 SHA-256 ，所以我們假設每一個私鑰 sk\^{}\{i\}\emph{\{j\}
都是 256 位元，然後每個公鑰 pk\^{}\{i\}}\{j\}也會是 256 位元。
...
\] 對於每一個位元 m\_i 如果 m\_i = 0 則該位元的簽章為
sk\^{}\{0\}\emph{\{i\}；如果 m\_i = 1 則該位元的簽章為 sk\^{}\{1\}}\{i\}
\[
\sigma_{i} =
\left\{
\begin{aligned}
...
\hypertarget{ux7a0bux5f0fux5be6ux4f5c}{%
\section{程式實作}\label{ux7a0bux5f0fux5be6ux4f5c}}
...
\hypertarget{ux5bc6ux9470ux751fux6210-1}{%
\subsection{密鑰生成}\label{ux5bc6ux9470ux751fux6210-1}}
...
\section{缺點}\label{ux7f3aux9ede}}

相信你很快發現，我們這個系統是針對每個位元進行簽章，而每個簽章都要使用
256 位元的私鑰以及公鑰，可見最後整個簽章肯定很佔空間：
...
其實「簽章大小很大」這點是 Hash-based signature 常見的缺點。通過 NIST
後量子密碼競賽的另一個基於晶格密碼學的簽章系統，Falcon，就有比較短的簽章大小。舉例：Falcon
的最高安全參數 Falcon-1024 系統，簽出來的簽章大小只有 1500 byte
以下（ref: https://openquantumsafe.org/liboqs/algorithms/sig/falcon）
...
\item
  \textbf{Hash 函數}
  是現代密碼學中不可缺少的工具，擁有不可逆性和抗碰撞性，使其能夠有效地保護數位訊息的完整性。
\item
  \textbf{量子計算的威脅}：Grover's Algorithm 能夠降低 Hash
  函數的安全性，因此我們需要考慮提高 Hash
  函數的位元數，以確保未來量子時代的安全性。
\item
  \textbf{LD-OTS} (一次性簽名方案) 利用 Hash
  函數來保證每個位元的安全性，但其簽章大小較大，這是使用 Hash-based
  signature 系統的常見問題。
\end{itemize}
