...
\hypertarget{ux56deux9867ux6628ux5929ux6cb9ux918bux91acux7c3dux7ae0}{%
\section{回顧昨天油醋醬簽章}\label{ux56deux9867ux6628ux5929ux6cb9ux918bux91acux7c3dux7ae0}}
...
\] 在簽章的時候，我們會隨機給醋變量 x1, \ldots, x\_v
一組隨機值，於是未知數只剩下紅色的油變量 \[
w_{i}
=
\sum_{j=1}^{v} \sum_{k=j}^{v}\alpha_{jk}^{(i)} x_{j}x_{k} 
+
...
\hypertarget{ux5c64ux6b21ux5316ux7684ux4e2dux9593ux6620ux5c04}{%
\section{層次化的中間映射}\label{ux5c64ux6b21ux5316ux7684ux4e2dux9593ux6620ux5c04}}
...
\] 請想像一下：假設一開始我們把前 v1
個變量先當作醋變量，並隨機生成他的值。把第 v1 + 1 到 v2
個變量先當作油變量，那我們可以透過 o1 個形如以下的 v1-o1
油醋多項式中求到這 o1 個油變量的值 \[
f^{(i)}
=
...
\] 到此為止我們一共知道了 v2 = v1 + o1 個變量的值。接著把接下來的 v2+1
到 v3 個變量當作油變量，那我們可以透過 o2 個形如以下的 v2-o2
油醋多項式中求到這 o2 個油變量的值 \[
f^{(i)}
=
...
以此類推，最終就可以求到所有的 x 變量。在這過程中，各層分別需要用到 o1 +
o2 + \ldots{} + o\_u = n - v1 個油醋多項式。

\hypertarget{ux5f69ux8679ux7c3dux7ae0}{%
\section{彩虹簽章}\label{ux5f69ux8679ux7c3dux7ae0}}

\hypertarget{ux96d9ux6975ux69cbux9020ux6cd5}{%
\subsection{雙極構造法}\label{ux96d9ux6975ux69cbux9020ux6cd5}}

生成兩個隨機的可逆仿射變換 S 與 T 中間映射為 F ，分別由 o1 個 v1-o1
油醋多項式、o2 個 v2-o2 油醋多項式、\ldots\ldots、o\_u 個 v\_u-o\_u
油醋多項式。一共有 n - v1 個油醋多項式。

私鑰為 S, F, T 公鑰為 P(x) = S(F(T(x)))

S 與 T，分別是作用在 n-v1 維與 n 維空間上。 \#\# 簽章過程
好！我們來看簽章過程！
...
y = \mathcal{F}^{-1}(x)
\]
這個部分是透過我們第二節「層次化的中間映射」中介紹的方法計算得到。其中 y
是 n 維度的向量。
...
\hypertarget{ux9a57ux7ae0ux904eux7a0b}{%
\subsection{驗章過程}\label{ux9a57ux7ae0ux904eux7a0b}}
...
\hypertarget{ux5f8cux8a18}{%
\section{後記}\label{ux5f8cux8a18}}

道歉啟事：今天收到兵單，心情極度鬱卒，因此沒有把程式寫好。（你可以在這裡看到月亮巨蟹人是有多容易被心情影響工作\ldots\ldots）

ref\\
DING, Jintai; GOWER, Jason E.; SCHMIDT, Dieter S.~\emph{Multivariate
