...
昨天我們討論了 RS 編碼，今天來討論他的錯誤更正機制。 回憶一下 Hamming
Code
，他可以更正一個錯誤，並且有個快速漂亮的演算法來修正該錯誤。所以今天我們也要討論兩個問題：
- RS 編碼可以更正幾個錯誤？ - RS 編碼如何更正錯誤？

\hypertarget{ux7c21ux8981ux8907ux7fd2-rs-ux7de8ux78bc}{%
\section{簡要複習 RS
編碼}\label{ux7c21ux8981ux8907ux7fd2-rs-ux7de8ux78bc}}
...
\hypertarget{ux66f4ux6b63ux6a5fux5236}{%
\section{更正機制}\label{ux66f4ux6b63ux6a5fux5236}}

現在假設傳遞過程中出現 e 個錯誤，我們等等會討論 e
必須在哪個範圍以內才能做更正。

意思是說，接收者收到的是 \[
y = (y_{1},\dots,y_{n})
\] 其中有 e 個 y\_i 是錯的，其他都滿足 \[
y_{i} = f_{m}(\alpha_{i})
\]

\hypertarget{ux932fux8aa4ux4f4dux7f6eux591aux9805ux5f0f-error-locator-polynomial}{%
\subsection{錯誤位置多項式 Error Locator
Polynomial}\label{ux932fux8aa4ux4f4dux7f6eux591aux9805ux5f0f-error-locator-polynomial}}
...
\]
這個多項式叫做錯誤位置多項式。錯誤位置多項式的特性是，它在每個錯誤位置~alpha\_i
都為
0，意思是說，通過計算~E(x)，我們能夠定位出訊息中出現錯誤的位置，這在後續的錯誤更正過程中至關重要

如果我們的編碼有 e 個錯誤，則 \[
E(x) = \prod_{i: y_{i}\neq f_{m}(\alpha_{i})} (x-\alpha_{i})
\] 會是 e 次，且領導係數等於1。

我們將使用這個錯誤位置多項式，來推導解決 RS
編碼中錯誤位置與內容的關鍵等式。

\hypertarget{ux4e00ux500bux95dcux9375ux7684ux7b49ux5f0f}{%
\subsection{一個關鍵的等式}\label{ux4e00ux500bux95dcux9375ux7684ux7b49ux5f0f}}
...
\hypertarget{ux6700ux5f8cux7684ux4e00ux4e9bux89c0ux5bdf}{%
\subsection{最後的一些觀察}\label{ux6700ux5f8cux7684ux4e00ux4e9bux89c0ux5bdf}}

\hypertarget{ux8a0aux606fux591aux9805ux5f0f}{%
\subsubsection{訊息多項式}\label{ux8a0aux606fux591aux9805ux5f0f}}
...
\hypertarget{ux5c0dux4ed8-nx-ux8207-ex}{%
\subsubsection{對付 N(x) 與 E(x)}\label{ux5c0dux4ed8-nx-ux8207-ex}}

如果我們的編碼有 e 個錯誤，則 \[
E(x) = \prod_{i: y_{i}\neq f_{m}(\alpha_{i})} (x-\alpha_{i})
\] 會是 e 次，且領導係數等於1。

你以為你這樣可以算 E 嗎？並沒有！因為我們不知道 y\_i 在哪個 i
出錯，所以不能直接算。
...
好！我們共有 e + (k+e) = k + 2e 個未知數，然後有 n
個條件，解出唯一解的條件就是： \[
n \ge k+2e
\] 移向得到 \[
e \le \frac{n-k}{2}
\] 這告訴我們，當 e 小於 (n-k)/2 時，RS 編碼才能進行更正。否則 e 大於
(n-k)/2 時，我們的未知數是比條件還要多，解不回唯一的 E(x) 與 N(x)
，也就解不回訊息多項式

\hypertarget{ux89e3ux95dcux9375ux7b49ux5f0fux7684ux7d30ux7bc0}{%
\subsection{解關鍵等式的細節}\label{ux89e3ux95dcux9375ux7b49ux5f0fux7684ux7d30ux7bc0}}
...
我們使用以下的 Channel function 來模擬雜訊，其中 num\_of\_error
是最大錯誤數量，因為我們這裡是 \[
e \le \frac{n-k}{2} = 2
\] 所以我們用 num\_of\_error = 2
...
\item
  RS
  編碼能更正最多~(n−k)/2\hspace{0pt}~個錯誤，當超過此範圍時，無法唯一解出錯誤位置多項式和訊息多項式。
\item
  錯誤位置多項式~E(x)~是用來定位錯誤符號位置的關鍵。它在所有錯誤位置上取值為
  0，幫助濾除錯誤數據。
\item
...
\item
  使用 SageMath 程式模擬 RS
  編碼的錯誤更正機制，透過編碼、通道噪聲及解碼步驟展示了實際應用。
\end{itemize}

RS 編碼與 Hamming Code
相比，能夠更正更多的錯誤，這使它成為數位通訊和數據存儲中極為重要的錯誤更正碼。特別是在高噪聲環境中，RS
編碼的錯誤更正能力顯得尤為強大，並被廣泛應用於光碟、衛星通訊等領域。
